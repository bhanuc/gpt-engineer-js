...
describe('collect_learnings tests', () => {
  test('collect_learnings', async () => {
    ...
    const steps = [genCode];
    const dbs = new DBs(new DB('/tmp'), new DB('/tmp'), new DB('/tmp'), new DB('/tmp'), new DB('/tmp'), new DB('/tmp'));
		dbs.input.set("prompt",  'test prompt\n with newlines')
		dbs.input.set("feedback", 'test feedback')
    const code = 'this is output\n\nit contains code';
    dbs.logs.set(genCode.name , JSON.stringify([{ role: 'system', content: code }]) );
