...
class DBs {
    ...
    constructor(memory: DB, logs: DB, preprompts: DB, input: DB, workspace: DB, archive: DB) {
        this.memory = memory;
        this.logs = logs;
...
function archive(dbs: DBs): void {
    ...
    copy(dbs.memory.path, path.join(dbs.archive.path, timestamp, path.basename(dbs.memory.path)));
    copy(dbs.workspace.path, path.join(dbs.archive.path, timestamp, path.basename(dbs.workspace.path)));
}
